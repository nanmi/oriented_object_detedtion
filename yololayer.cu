...
namespace nvinfer1
{
        ...
        const int netwidth, const int netheight, int maxoutobject, int yoloWidth, int yoloHeight, const float anchors[CHECK_COUNT * 2], int classes, int outputElem)
    {
        ...
        int info_len_i = 5 + classes + angles;
        ...
        for (int k = 0; k < 3; ++k) {
            ...
            float max_ang_prob = 0.0;
            for (int i = 5; i < info_len_i - angles; ++i) {
                float cls_p = Logist(curInput[idx + k * info_len_i * total_grid + i * total_grid]);
                if (cls_p > max_cls_prob) {
                    max_cls_prob = cls_p;
                    class_id = i - 5; // 0 ~ 79 classes
                }
            }
            for (int j = 5 + classes; j < info_len_i; ++j) {
                float ang_p = Logist(curInput[idx + k * info_len_i * total_grid + j * total_grid]);
                if (ang_p > max_ang_prob) {
                    max_ang_prob = ang_p;
                    angle_id = j - classes - 5; //0 ~ 179°
                }
            }
