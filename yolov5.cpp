...
ICudaEngine* createEngine_s(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../ood_yolov5s.wts");
    ...
    IConvolutionLayer* det0 = network->addConvolutionNd(*bottleneck_csp17->getOutput(0), 3 * (Yolo::CLASS_NUM + 5 + Yolo::ANGLE), DimsHW{ 1, 1 }, weightMap["model.24.m.0.weight"], weightMap["model.24.m.0.bias"]);
    ...
    IConvolutionLayer* det1 = network->addConvolutionNd(*bottleneck_csp20->getOutput(0), 3 * (Yolo::CLASS_NUM + 5 + Yolo::ANGLE), DimsHW{ 1, 1 }, weightMap["model.24.m.1.weight"], weightMap["model.24.m.1.bias"]);
    ...
    IConvolutionLayer* det2 = network->addConvolutionNd(*bottleneck_csp23->getOutput(0), 3 * (Yolo::CLASS_NUM + 5 + Yolo::ANGLE), DimsHW{ 1, 1 }, weightMap["model.24.m.2.weight"], weightMap["model.24.m.2.bias"]);
...
int main(int argc, char** argv) {
    ...
    engine_name = "../yolov5" + engine_name + "_ood_obb_fp16.engine";
    if (argc == 2 && std::string(argv[1]) == "-s") {
        ...
        std::cerr << "./ood_yolov5 -s  // serialize model to plan file" << std::endl;
        std::cerr << "./ood_yolov5 -d ../samples  // deserialize plan file and run inference" << std::endl;
        return -1;
    }
    ...
    for (int f = 0; f < (int)file_names.size(); f++) {
        ...
        for (int b = 0; b < fcount; b++) {
            ...
            for (size_t j = 0; j < res.size(); j++) {
                ...
                int center_x = r.x + (int)(r.width / 2);
                int center_y = r.y + (int)(r.height / 2);
                cv::RotatedRect rRect = cv::RotatedRect(cv::Point2f(center_x,center_y), cv::Size2f(r.width,r.height), (float)(res[j].angle_id));
                cv::Point2f vertices[4];
                rRect.points(vertices);
                for (int ii = 0; ii < 4; ii++){
                    cv::line(img, vertices[ii], vertices[(ii+1)%4], cv::Scalar(0,0,255), 2);
                }

                cv::putText(img, std::to_string((int)res[j].angle_id), cv::Point(r.x + 20, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.8, cv::Scalar(0x00, 0x00, 0xFF), 2);
                // cv::rectangle(img, r, cv::Scalar(0x27, 0xC1, 0x36), 2);
                cv::putText(img, std::to_string((int)res[j].class_id), cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.8, cv::Scalar(0xFF, 0x00, 0x00), 2);
